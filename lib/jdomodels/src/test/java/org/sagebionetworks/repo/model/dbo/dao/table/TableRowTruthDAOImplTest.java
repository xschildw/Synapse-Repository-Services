...
public class TableRowTruthDAOImplTest {
	...
	public void testDoubles() throws IOException, NotFoundException {
		...
		RawRowSet rowSet = new RawRowSet(TableModelUtils.getHeaders(mapper.getColumnModels()), null, tableId, rows);
		// Append this change set
		RowReferenceSet refSet = tableRowTruthDao.appendRowSetToTable(creatorUserGroupId, tableId, mapper, rowSet);
		assertNotNull(refSet);
		// Get the rows back
		RowSet fetched = tableRowTruthDao.getRowSet(tableId, 0l, ALL_SET, mapper);
	...
	public void testListRowSetsForTable() throws IOException{
		...
		assertNotNull(refSet);
		// Add some more rows
		set = new RawRowSet(set.getHeaders(), set.getEtag(), set.getTableId(), TableModelTestUtils.createRows(mapper.getColumnModels(), 2));
	...
	public void testGetRowSet() throws IOException, NotFoundException{
		...
		fetched = tableRowTruthDao.getRowSet(tableId, 0l,
				Sets.newHashSet(rows.get(0).getRowId(), rows.get(2).getRowId(), rows.get(4).getRowId()), mapper);
		assertNotNull(fetched);
		assertEquals(mapper.getSelectColumns(), fetched.getHeaders());
		assertEquals(tableId, fetched.getTableId());
		assertNotNull(fetched.getEtag());
		...
		try{
			tableRowTruthDao.getRowSet(tableId, 1l, ALL_SET, mapper);
			fail("Should have failed");
	...
	public void testGetRowSetOriginalsMultiple() throws IOException, NotFoundException {
		...
		// add row
		set = new RawRowSet(set.getHeaders(), set.getEtag(), set.getTableId(), TableModelTestUtils.createRows(mapper.getColumnModels(), 1));
		tableRowTruthDao.appendRowSetToTable(creatorUserGroupId, tableId, mapper, set);
		...
		RawRowSet toUpdateOneRaw = new RawRowSet(set.getHeaders(), toUpdateOne.getEtag(), set.getTableId(), Lists.newArrayList(toUpdate));
...
.newArrayList(mapper.getColumnModels().get(3), mapper.getColumnModels().get(0)), false));
	...
	public void testAppendRowsUpdate() throws IOException, NotFoundException{
		...
		RowSet toUpdate = tableRowTruthDao.getRowSet(tableId, 0l, ALL_SET, mapper);
		// remove a few rows
		...
		RawRowSet toUpdateOneRaw = new RawRowSet(set.getHeaders(), toUpdate.getEtag(), toUpdate.getTableId(), toUpdate.getRows());
		...
		RowSet updated = tableRowTruthDao.getRowSet(tableId, 1l, ALL_SET, mapper);
	...
	public void testAppendRowsUpdateAndGetLatest() throws Exception {
		...
		toUpdate = tableRowTruthDao.getRowSet(tableId, 1l, ALL_SET, mapper);
		// remove a few rows
		...
		// Now append the changes.
		toUpdateOneRaw = new RawRowSet(set.getHeaders(), toUpdate.getEtag(), toUpdate.getTableId(), toUpdate.getRows());
		refSet = tableRowTruthDao.appendRowSetToTable(creatorUserGroupId, tableId, mapper, toUpdateOneRaw);
		assertNotNull(refSet);
		for (RowReference ref : refSet.getRows()) {
			rowVersions.put(ref.getRowId(), ref.getVersionNumber());
	...
	public void testCacheBehindCheck() throws Exception {
		...
		// Append this change set
		for (int i = 0; i < 3; i++) {
			RawRowSet set = new RawRowSet(TableModelUtils.getHeaders(mapper.getColumnModels()), null, tableId,
	...
	public void testAppendDeleteRows() throws IOException, NotFoundException {
		...
		// Now fetch the rows for an update
		RowSet toUpdate1 = tableRowTruthDao.getRowSet(tableId, 0l, ALL_SET, mapper);
		// For this case each update will change a different row so there is no conflict.
		...
		RawRowSet toUpdate1Raw = new RawRowSet(set.getHeaders(), toUpdate1.getEtag(), toUpdate1.getTableId(), toUpdate1.getRows());
		tableRowTruthDao.appendRowSetToTable(creatorUserGroupId, tableId, mapper, toUpdate1Raw);

		// Now fetch the rows for an update
		RowSet toUpdate2 = tableRowTruthDao.getRowSet(tableId, 0l, ALL_SET, mapper);
		...
		RawRowSet toUpdate2Raw = new RawRowSet(set.getHeaders(), toUpdate2.getEtag(), toUpdate2.getTableId(), toUpdate2.getRows());
		tableRowTruthDao.appendRowSetToTable(creatorUserGroupId, tableId, mapper, toUpdate2Raw);

		// Now fetch the rows for an update
		RowSet toUpdate3 = tableRowTruthDao.getRowSet(tableId, 1l, ALL_SET, mapper);
		// delete second (was third) row and update only that one
		deletion = new Row();
		...
		RawRowSet toUpdate3Raw = new RawRowSet(set.getHeaders(), toUpdate3.getEtag(), toUpdate3.getTableId(), toUpdate3.getRows());
		...
		RowSet rowSetBefore = tableRowTruthDao.getRowSet(tableId, 0L, ALL_SET, mapper);
		RowSet rowSetAfter = tableRowTruthDao.getRowSet(tableId, 1L, ALL_SET, mapper);
		RowSet rowSetAfter2 = tableRowTruthDao.getRowSet(tableId, 2L, ALL_SET, mapper);
		RowSet rowSetAfter3 = tableRowTruthDao.getRowSet(tableId, 3L, ALL_SET, mapper);
	...
	public void testTableRowsDelete() throws IOException, NotFoundException {
		...
		for (int i = 0; i < COUNT; i++) {
			tableRowTruthDao.getRowSet(tableId, i, ALL_SET, mapper);
		}

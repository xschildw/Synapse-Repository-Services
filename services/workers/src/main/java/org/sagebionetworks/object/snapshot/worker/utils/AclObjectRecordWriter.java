...
public class AclObjectRecordWriter implements ObjectRecordWriter {
	...
	public void buildAndWriteRecords(ProgressCallback progressCallback, List<ChangeMessage> messages) throws IOException {
		...
		List<AclKinesisLogRecord> kinesisToWrite = new ArrayList<>(messages.size());
		for (ChangeMessage message : messages) {
			...
			try {
				...
				AclKinesisLogRecord aclKinesisLogRecord = new AclKinesisLogRecord().withAclRecord(record).withTimestamp(message.getTimestamp().getTime());
